...
var Sortable = Ember.Mixin.create(Droppable, {
  ...
  /**
   * Resets drop properties on drag leave.
   *
   * Since dragLeave triggers when you move from parent to child
   * elements, and `dragLeave` always triggers after `dragEnter`, we
   * track `lastEntered` on the window `dragenter` event and then make
   * sure we're actually leaving this element
   *
   * @method resetDropPropsOnLeave
   * @private
   */

  resetDropPropsOnLeave: function(event) {
    var el = this.get('element');
    var actuallyLeaving = el !== lastEntered && !el.contains(lastEntered);
    if (actuallyLeaving) {
      // this work will probably be done in updateDropTarget, but in case
      // the user drags out and not into another valid drop target
      // (outside the window, etc.) do this as a safeguard
